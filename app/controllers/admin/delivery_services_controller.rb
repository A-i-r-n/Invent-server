  ...
  class DeliveryServicesController < Admin::BaseController
    ...
    def create
      ...
      if @delivery_service.save
        redirect_to [:admin,:delivery_services], flash: { notice: t('shoppe.delivery_services.create_notice') }
      else
        render action: 'new'
      end
    end
    ...
    def update
      if @delivery_service.update(safe_params)
        redirect_to [:edit,:admin, @delivery_service], flash: { notice: t('shoppe.delivery_services.update_notice') }
      else
        render action: 'edit'
      end
    end
