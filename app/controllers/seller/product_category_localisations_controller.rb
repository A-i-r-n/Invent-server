require 'globalize'
module Seller
  class ProductCategoryLocalisationsController < Seller::ApplicationController
    before_filter { @active_nav = :product_categories }
    before_filter { @product_category = ProductCategory.find(params[:product_category_id]) }
    ...
    def update
      if @localisation.update(safe_params)
        redirect_to [:seller, @product_category, :localisations], notice: t('shoppe.localisations.localisation_updated')
      else
        render action: 'form'
      end
    end
