          ...
          category.children.ordered.each do |child|
            ...
            if child == current_category
              if link_to_current == false
                ...
                s << "#{nested_product_category_spacing_adjusted_for_depth child, relative_depth} &#8627; #{link_to(child.name, [:edit,:admin, child]).html_safe} (#{t('shoppe.product_category.nesting.current_category')})"
              end
            else
              s << "#{nested_product_category_spacing_adjusted_for_depth child, relative_depth} &#8627; #{link_to(child.name, [:edit,:admin, child]).html_safe}"
            end
            s << (link_to I18n.t('shoppe.products.products'), admin_products_path(category_id: child.id), style: 'float: right')
            s << '</td>'
            s << '</tr>'
