module Seller
  class OrdersController < Seller::ApplicationController
    ...
    def update
      ...
      if !request.xhr? && @order.update_attributes(safe_params)
        redirect_to [:seller,@order], flash: { notice: t('shoppe.orders.update_notice') }
      else
        render action: 'edit'
      end
    end
    ...
    def accept
      ...
      redirect_to [:seller,@order], flash: { notice: t('shoppe.orders.accept_notice') }
    ...
    def reject
      ...
      redirect_to [:seller,@order], flash: { notice: t('shoppe.orders.reject_notice') }
    rescue Shoppe::Errors::PaymentDeclined => e
    ...
    def ship
      ...
      redirect_to [:seller,@order], flash: { notice: t('shoppe.orders.ship_notice') }
    end
