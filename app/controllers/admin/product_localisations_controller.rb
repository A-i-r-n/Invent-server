require 'globalize'
module Admin
  class ProductLocalisationsController < Admin::ApplicationController
    ...
    def create
      if I18n.available_locales.include? safe_params[:locale].to_sym
        ...
        if @localisation.save
          ...
          redirect_to [:admin,@product, :localisations], flash: { notice: t('shoppe.localisations.localisation_created') }
        else
          render action: 'form'
        end
      else
        redirect_to [:admin,@product, :localisations]
      end
    end
    ...
    def update
      if @localisation.update(safe_params)
        redirect_to [:admin,@product, :localisations], flash: { notice: t('shoppe.localisations.localisation_updated') }
      else
        render action: 'form'
      end
    end
    ...
    def destroy
      ...
      redirect_to [:admin,@product, :localisations], flash: { notice: t('shoppe.localisations.localisation_destroyed') }
    end
