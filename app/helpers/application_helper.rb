    ...
    def attachment_preview(attachment, options = {})
      if attachment.present? && attachment.token.present?
        ''.tap do |s|
          ...
          s << "<span class='delete'>"
          s << link_to(t('helpers.attachment_preview.delete', default: 'Delete this file?'), admin_attachment_path(attachment.token), method: :delete, data: { confirm: t('helpers.attachment_preview.delete_confirm', default: 'Are you sure you wish to remove this attachment?') })
          s << '</span>'
          s << '</div>'
          s << '</div>'
        end.html_safe
