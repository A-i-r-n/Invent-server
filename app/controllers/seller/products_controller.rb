module Seller
  class ProductsController < Seller::ApplicationController
    ...
    def create
      ...
      if @product.save
        redirect_to [:seller,:products], flash: { notice: t('shoppe.products.create_notice') }
      else
        render action: 'new'
      end
    end
    ...
    def update
      if @product.update(safe_params)
        redirect_to [:edit,:seller, @product], flash: { notice: t('shoppe.products.update_notice') }
      else
        render action: 'edit'
      end
    end
    ...
    def import
      if request.post?
        if params[:import].nil?
          redirect_to import_seller_products_path, flash: { alert: t('shoppe.imports.errors.no_file') }
        else
          Product.import(params[:import][:import_file])
          redirect_to seller_products_path, flash: { notice: t('shoppe.products.imports.success') }
        end
      end
    end
